...
**SAE-feature attribution (sub-spec 5, v1.5).** `SAEFeatureRunner(model, sae_sites, resolver).run(clean_inputs, corrupted_inputs, metric, *, graddrop=False, include_error_node=False, max_features=None)` → `AtPResult` of per-**SAE-feature** attribution scores (`circuitry.patching.sae_features`). `sae_sites` maps `Site(component="resid_post", layer=L)` → a SAELens `SAE` (or a `(release, sae_id)` tuple loaded via `sae.load_sae`). Mechanism — **error-term substitution** (Marks "Sparse Feature Circuits"): on the clean forward at each site, `f = sae.encode(a)`, `x_hat = sae.decode(f)`, `eps = (a − x_hat).detach()` (the *frozen clean reconstruction error*), and the residual is replaced by `recon = x_hat + eps` (numerically lossless to ≈1e-7, so the model output is preserved) — which makes each SAE feature `f_i` a differentiable node. Scoring mirrors `mlp_neuron` in the SAE basis: `score(feature i) = Σ_pos(Δf_i ⊙ gradf_i)`, `Δf = f_corrupt − f_clean`, gradient taken at the clean activation; `graddrop=True` uses `Σ|per-position contribution|`. Features are enumerated where `Δf ≠ 0` (the **union** of clean-active and corrupted-active — a clean-inactive/corrupted-active feature has a real nonzero effect and must not be dropped); omitted features have score 0 by construction. `include_error_node=True` (opt-in, **default off**) additionally scores the reconstruction-error term as a first-class `sae_error` node via an **independent error leaf** (`err_leaf = eps.clone().requires_grad_(True); recon = x_hat + err_leaf`), so feature and error gradients come from a single forward + backward — feature scores are bit-identical with the node on or off (a *non*-detached error would algebraically cancel `x_hat` and zero `f.grad`). `bruteforce_feature_scores()` is the independent ground-truth path (a real forward intervention patching `f_i` clean→corrupted inside the splice). **On a model with a linear downstream, the analytic score equals brute-force feature patching at 1e-4** — exactness needs *downstream* linearity, not SAE linearity (`decode` is affine in `f`, and the detached `eps` keeps `recon` affine in `f`). New `Node` kinds: `sae_feature` (reuses the `neuron` field as the feature index) and `sae_error`; the edge-graph machinery (`_order`/`build_graph`) is untouched. Scope: HF-eager + `resid_post` sites only; `TLSiteResolver`, non-`resid_post` sites, and transcoder / matching_pursuit / temporal SAEs raise `NotImplementedError`. Supported SAE architectures: `standard` / `topk` / `jumprelu` (covers BatchTopK / Matryoshka at inference) / `gated`. The metric must be the differentiable tensor-returning variant (`logit_diff_t` / `kl_divergence_t` / `ce_loss_t`); a `.detach()`-ing float metric raises a clear `f.grad is None` error. Feature→feature **edges** + circuit extraction ship in sub-spec 6 (v1.6).
...
**Circuit extraction / pruning (v1.6).** `SAEFeatureCircuit.ranked()/top_k(n)/threshold(tau)` (copied from `EAPResult`) and `prune(method="threshold"|"acdc"|"both", tau, ablation_mode="corrupted"|"zero"|"mean")`. **Ablation is NODE-set, not edge-level** — downstream features share one residual `a_D`, so an edge cannot be ablated independently; instead non-circuit feature entries are replaced (corrupted/zero/mean) before `decode` at each spliced site, propagated forward. `faithfulness(C) = (m(C) − m(∅)) / (m(M) − m(∅))` and `completeness` via the complement (Marks §3.2; the metric-diff form can exceed [0,1] under anti-correlation — documented, not a bug). `FeatureACDCRunner(model, sae_sites, resolver).run(…, tau, ablation_mode, eap_skip_threshold)` is **greedy reverse-topological NODE pruning** (later sites first; weakest |node score| first; accept a removal if `KL_new − KL_current < tau`; `sweep(taus) → [(tau, n_kept, final_kl)]` Pareto). It reuses `ACDCRunner`'s control flow + `core.patching.kl_divergence`; the feature-basis set-ablation forward is net-new (`ACDCRunner._run_capturing_live` injects d_model per-head deltas and calls `_order`, so it is not reusable). `graph.py` / `EdgeGraph` / `_order` / `build_graph` are **untouched** — the feature graph is a dedicated `SAEFeatureEdgeGraph` keyed on `(site forward-order, feature index)`. Deferred: integrated-gradients edge variant (`ig`; the `variant` enum slot is reserved), per-position edges, `mlp_out`/`attn_out` sites, the TL backend.
...
- Logit lens / tuned lens beyond `core/lens.py`'s `logit_lens_kl`. **SAE-feature circuits shipped: node-level attribution in v1.5** (`SAEFeatureRunner`) and **feature→feature edges + greedy `FeatureACDC` in v1.6** (`SAEFeatureEdgeRunner`, §4.6); SAE *reconstruction* metrics shipped in v0.9 (`circuitry.sae`). Deferred SAE-circuit follow-ons: the integrated-gradients edge variant, per-position edges, `mlp_out`/`attn_out` sites, and the TransformerLens backend.
