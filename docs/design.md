...
`circuitry` is statistical diagnostics on weights / activations / gradients, usable live during training or post-hoc on saved checkpoints. The statistical core remains modality-agnostic; an opt-in interventional **activation-patching / attribution** pillar (EAP, AtP\*, ACDC — §4.6), SAE-reconstruction metrics (`circuitry.sae`), and node-level **SAE-feature attribution** (`SAEFeatureRunner`, v1.5) have since shipped. Tuned lens and feature→feature SAE circuit *edges* (the full sparse-feature graph) remain future work. The name is borrowed from electronics, not from interpretability research. The README MUST open with a one-line scope statement so users arriving from mechanistic-interpretability work understand what this is and where it's heading.
...
Attribution methods (EAP, AtP\*, ACDC — v1.0) and node-level SAE-feature attribution (`SAEFeatureRunner`, v1.5) build on this primitive; feature→feature SAE *edges* remain future work.
...
- Logit lens / tuned lens beyond `core/lens.py`'s `logit_lens_kl`. **Node-level SAE-feature attribution shipped in v1.5** (`SAEFeatureRunner`, §4.6); SAE *reconstruction* metrics shipped in v0.9 (`circuitry.sae`). Feature→feature SAE circuit **edges** (the full sparse-feature graph) remain future work.
...
- **SAE feature attribution is post-hoc, not a live diagnostic.** `SAEFeatureRunner` (§4.6, v1.5) runs on a clean/corrupted prompt pair *outside* the training loop, so it does not count against the ≤10% training-loop budget. Per site the splice adds ~2 matmuls (encode + decode) per forward and the run uses a single backward; the opt-in `sae_error` node adds **no** extra pass (independent error leaf).
