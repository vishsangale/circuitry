...
1. **Corrupted forward** — cache each node's residual-stream *output* (no grad).
2. **Clean forward + backward** — cache each node's *output* activation **and**
   the gradient of the metric w.r.t. each *reader slot's residual-stream input*.
3. **Analytic scoring** — for every edge, a cheap dot product (§5). No per-edge
   forward passes; cost is `O(forwards + backwards) + O(edges)` cheap ops.
...
## 4. Model support — reader-side gradients

The hard requirement EAP adds beyond the primitive: for each reader `(head,
slot)` (and `mlp_in`, `logits_in`), the gradient of the metric w.r.t. **that
reader's residual-stream input**, in residual (`d_model`) space, kept separate
per reader-slot so edges are typed.
...
directly — no manual back-mapping, no explicit LN handling. Writer outputs:
`blocks.{L}.attn.hook_z` (per head), `blocks.{L}.hook_mlp_out`, `hook_embed`.
...
- **Writer side** (node output activations) reuses the primitive's
  `HFSiteResolver` (`attn_head_out` = per-head `o_proj` input; `mlp_out`;
  embed weight).
